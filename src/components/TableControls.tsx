  ...
  return (
    <div className="flex w-max flex-col gap-2">
        ...
        {fields.map((columnIdField, index) => {
          return (
            <div key={columnIdField.id} className="flex items-center gap-2 rounded bg-slate-700 p-1 pl-2">
              ...
              <button onClick={() => remove(index)} className="flex h-8 w-8 items-center justify-center rounded bg-red-500 bg-opacity-50 p-2 hover:bg-red-400 hover:bg-opacity-50">
      ...
      <Listbox onChange={append} as="div" className="relative w-max">
        {({ open }) => (
          <>
            <Listbox.Button className="flex cursor-pointer items-center justify-center gap-2 rounded bg-lime-800 px-2 py-1 hover:bg-lime-700">
    ...
    <Disclosure as="div" className="mb-2 w-full">
      {({ open }) => (
        <>
          ...
          <Disclosure.Panel className="flex flex-col rounded bg-slate-700 bg-opacity-20 p-2">
            <GroupForm />
          </Disclosure.Panel>
        </>
      )}
